...
class ModrinthSearchViewModel: ObservableObject {
    ...
    func searchMods(query: String, page: Int = 1) async {
        isLoading = true
        error = nil
        do {
    ...
    var body: some View {
        ZStack {
            if viewModel.isLoading {
                ProgressView("game.version.loading")
        ...
        .onAppear {
            if !hasLoaded {
                hasLoaded = true
                Task { await viewModel.searchMods(query: query, page: currentPage) }
            }
        }
        .onChange(of: currentPage) { _, newPage in
            Task { await viewModel.searchMods(query: query, page: newPage) }
        }
        .onChange(of: query) { _, newQuery in
            currentPage = 1
            Task { await viewModel.searchMods(query: newQuery, page: 1) }
        }
